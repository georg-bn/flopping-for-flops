...
def get_tags(model: str):
    ...
    if 's2' in model.lower():
        tags.append('Equivariant')
    else:
        tags.append('Baseline')
    return tags

def get_model_prefix(model: str):
    if 'deit' in model.lower():
        return 'ViT'
    elif 'resmlp' in model.lower():
        return 'mlp'
...
def get_size(model:str):
    if 'tiny' in model.lower() or model == 'resmlp_equi_s2_12':
        return 'T'
    elif 'small' in model.lower() or model == 'resmlp_equi_s2_24': 
        return 'S'
    elif 'base' in model.lower() or model == 'resmlp_equi_s2_B_24':
        return 'B'
    elif 'large' in model.lower() or model == 'resmlp_equi_s2_L_24':
        return 'L'
    elif 'huge' in model.lower():
        return 'H'
    return ''

def format_name(model):
    equi_part = 's2_' if 's2' in model else ''

    model_type = get_model_prefix(model)
    
    size = get_size(model)

    return equi_part+model_type+"_"+size
