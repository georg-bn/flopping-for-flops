...
from hybrid_models_v2 import (
    hybrid_deit_base_patch16_LS,
)
...
if __name__ == "__main__":
    ...
    for precision in ["high"]:  # 'highest', 'high', 'medium'
        ...
        for name, comp in itertools.product(
            ...
            if name == "hybrid":
                net = hybrid_deit_base_patch16_LS(img_size=img_size).to(device)
            elif name == "standard":
                net = deit_base_patch16_LS(img_size=img_size).to(device)
            else:
                raise ValueError()
            print(f"Profiling {name}")
            if comp:
