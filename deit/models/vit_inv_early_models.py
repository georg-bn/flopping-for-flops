...
class d2_inv_early_vit_models(nn.Module):
    """ D2-equivariant in early layers, then invariarize to only invariant features.
    ...
    def forward_features(self, x):
        ...
        # Invariarize (-1)-equi feats in xs[1].
        # We actually obtain a "complete" invariant here.
        x = torch.cat(
            (xs[0], torch.abs(xs[1])),
            dim=-1,
        )
...
@register_model
def d2_inv_early_deit_tiny_patch16_LS(pretrained=False,
                         img_size=224,
                         pretrained_21k = False,
                         **kwargs):
    model = d2_inv_early_vit_models(
        img_size = img_size,
        patch_size=16,
        embed_dim=192,
        depth=12,
...
@register_model
def d2_inv_early_deit_small_patch16_LS(pretrained=False,
                          img_size=224,
                          pretrained_21k = False,
                          **kwargs):
    model = d2_inv_early_vit_models(
...
@register_model
def d2_inv_early_deit_medium_patch16_LS(pretrained=False,
                                  img_size=224,
                                  pretrained_21k = False,
                                  **kwargs):
    model = d2_inv_early_vit_models(
...
@register_model
def d2_inv_early_deit_base_patch16_LS(pretrained=False, img_size=224, pretrained_21k = False,  **kwargs):
    model = d2_inv_early_vit_models(
        img_size=img_size,
        patch_size=16,
        embed_dim=768,
...
@register_model
def d2_inv_early_deit_large_patch16_LS(pretrained=False,
                                 img_size=224,
                                 pretrained_21k = False,
                                 **kwargs):
    model = d2_inv_early_vit_models(
...
@register_model
def d2_inv_early_deit_huge_patch14_LS(pretrained=False,
                                img_size=224,
                                pretrained_21k = False,
                                **kwargs):
    model = d2_inv_early_vit_models(
...
@register_model
def d2_inv_early_deit_huge_patch14_52_LS(pretrained=False,
                            img_size=224,
                            pretrained_21k = False,
                            **kwargs):
    model = d2_inv_early_vit_models(
